import { useForm, useWatch } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { useState } from 'react'
import { newFieldSchema, type NewFieldSchema } from '../schemas/fieldSchema'
import { FIELD_TYPES } from '../constants/fields'
import type { FieldOption } from '../types/form'
import { FieldImportDialog } from './FieldImportDialog'
import {
  Card,
  SectionTitle,
  Form,
  FormGroup,
  Label,
  Input,
  Select,
  Button,
  ErrorText,
  CheckboxWrapper,
  Row,
} from '../styles'

interface FieldBuilderProps {
  onAddField: (data: {
    label: string
    type: string
    placeholder?: string
    required: boolean
    defaultValue?: string
    minLength?: number
    maxLength?: number
    min?: number
    max?: number
    pattern?: string
    options?: FieldOption[]
  }) => void
  onImport: (json: string, mode: 'replace' | 'append') => void
}

export function FieldBuilder({ onAddField, onImport }: FieldBuilderProps) {
  const [showImport, setShowImport] = useState(false)
  const {
    register,
    handleSubmit,
    reset,
    control,
    formState: { errors },
  } = useForm<NewFieldSchema>({
    resolver: zodResolver(newFieldSchema),
    defaultValues: { required: false },
  })

  const selectedType = useWatch({ control, name: 'type' })
  const showOptions = selectedType === 'select' || selectedType === 'radio'

  const [optionEntries, setOptionEntries] = useState<string[]>([])

  const onSubmit = (data: NewFieldSchema) => {
    const options: FieldOption[] = optionEntries
      .map(line => line.trim())
      .filter(Boolean)
      .map(line => {
        const [value, ...labelParts] = line.split(',')
        return { value: value.trim(), label: labelParts.join(',').trim() || value.trim() }
      })

    onAddField({
      label: data.label,
      type: data.type,
      placeholder: data.placeholder || undefined,
      required: data.required ?? false,
      defaultValue: data.defaultValue || undefined,
      minLength: data.minLength ? Number(data.minLength) : undefined,
      maxLength: data.maxLength ? Number(data.maxLength) : undefined,
      min: data.min ? Number(data.min) : undefined,
      max: data.max ? Number(data.max) : undefined,
      pattern: data.pattern || undefined,
      options: options.length > 0 ? options : undefined,
    })

    reset()
    setOptionEntries([])
  }

  const addOptionEntry = () => setOptionEntries(prev => [...prev, ''])
  const updateOptionEntry = (index: number, value: string) =>
    setOptionEntries(prev => prev.map((e, i) => (i === index ? value : e)))
  const removeOptionEntry = (index: number) =>
    setOptionEntries(prev => prev.filter((_, i) => i !== index))

  return (
    <Card>
      <SectionTitle style={{ marginBottom: 20 }}>
        Adicionar Novo Campo
      </SectionTitle>
      <Form onSubmit={handleSubmit(onSubmit)}>
        <FormGroup>
          <Label htmlFor="newFieldLabel">Label do campo</Label>
          <Input
            id="newFieldLabel"
            type="text"
            placeholder="Ex: Sobrenome"
            {...register('label')}
          />
          {errors.label && <ErrorText>{errors.label.message}</ErrorText>}
        </FormGroup>

        <FormGroup>
          <Label htmlFor="newFieldType">Tipo do campo</Label>
          <Select id="newFieldType" {...register('type')}>
            <option value="">Selecione um tipo...</option>
            {FIELD_TYPES.map(ft => (
              <option key={ft.value} value={ft.value}>{ft.label}</option>
            ))}
          </Select>
          {errors.type && <ErrorText>{errors.type.message}</ErrorText>}
        </FormGroup>

        <FormGroup>
          <Label htmlFor="newFieldPlaceholder">Placeholder (opcional)</Label>
          <Input
            id="newFieldPlaceholder"
            type="text"
            placeholder="Ex: Digite seu sobrenome"
            {...register('placeholder')}
          />
          {errors.placeholder && <ErrorText>{errors.placeholder.message}</ErrorText>}
        </FormGroup>

        <CheckboxWrapper>
          <input type="checkbox" id="newFieldRequired" {...register('required')} />
          <Label htmlFor="newFieldRequired" style={{ margin: 0 }}>Campo obrigatório</Label>
        </CheckboxWrapper>

        <FormGroup>
          <Label htmlFor="newFieldDefault">Valor padrão (opcional)</Label>
          <Input
            id="newFieldDefault"
            type="text"
            placeholder="Ex: valor@padrao.com"
            {...register('defaultValue')}
          />
        </FormGroup>

        <Row>
          <FormGroup>
            <Label>Mín. caracteres</Label>
            <Input
              type="number"
              placeholder="Ex: 3"
              {...register('minLength')}
            />
            {errors.minLength && <ErrorText>{errors.minLength.message}</ErrorText>}
          </FormGroup>
          <FormGroup>
            <Label>Máx. caracteres</Label>
            <Input
              type="number"
              placeholder="Ex: 50"
              {...register('maxLength')}
            />
            {errors.maxLength && <ErrorText>{errors.maxLength.message}</ErrorText>}
          </FormGroup>
        </Row>

        <Row>
          <FormGroup>
            <Label>Valor mínimo</Label>
            <Input
              type="number"
              placeholder="Ex: 18"
              {...register('min')}
            />
          </FormGroup>
          <FormGroup>
            <Label>Valor máximo</Label>
            <Input
              type="number"
              placeholder="Ex: 120"
              {...register('max')}
            />
          </FormGroup>
        </Row>

        <FormGroup>
          <Label htmlFor="newFieldPattern">Regex de validação (opcional)</Label>
          <Input
            id="newFieldPattern"
            type="text"
            placeholder="Ex: ^\([0-9]{2}\) [0-9]{5}-[0-9]{4}$"
            style={{ fontFamily: 'monospace', fontSize: 12 }}
            {...register('pattern')}
          />
        </FormGroup>

        {showOptions && (
          <FormGroup>
            <Label>Opções (valor, Label)</Label>
            {optionEntries.map((entry, i) => (
              <div key={i} style={{ display: 'flex', gap: 8 }}>
                <Input
                  type="text"
                  placeholder="Ex: M, Masculino"
                  value={entry}
                  onChange={e => updateOptionEntry(i, e.target.value)}
                />
                <Button type="button" onClick={() => removeOptionEntry(i)} style={{ flex: 0, padding: '0.5rem 0.75rem', background: '#ba1a1a' }}>
                  X
                </Button>
              </div>
            ))}
            <Button type="button" onClick={addOptionEntry} style={{ background: '#6b7280' }}>
              + Adicionar opção
            </Button>
          </FormGroup>
        )}

        <Button type="submit">Adicionar Campo</Button>
      </Form>

      <div style={{ marginTop: 16 }}>
        <Button
          type="button"
          onClick={() => setShowImport(true)}
          style={{ width: '100%', background: '#6b7280' }}
        >
          Importar JSON
        </Button>
      </div>

      {showImport && (
        <FieldImportDialog
          onImport={onImport}
          onClose={() => setShowImport(false)}
        />
      )}
    </Card>
  )
}
